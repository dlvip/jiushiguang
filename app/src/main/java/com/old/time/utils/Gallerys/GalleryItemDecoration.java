package com.old.time.utils.Gallerys;
...
public class GalleryItemDecoration extends RecyclerView.ItemDecoration {
    ...
    private void onSetVerticalParams(ViewGroup parent, View itemView, int position, int itemCount) {
        ...
        int itemNewHeight = parent.getHeight() - UIHelper.dip2px(4 * mPageMargin + 2 * mLeftPageVisibleWidth);

        mItemComusemY = itemNewHeight + UIHelper.dip2px(2 * mPageMargin);


        DebugLog.d(TAG, "itemNewHeight=" + itemNewHeight);

        // 适配第0页和最后一页没有左页面和右页面，让他们保持左边距和右边距和其他项一样
        int topMargin = position == 0 ? UIHelper.dip2px(mLeftPageVisibleWidth + 2 * mPageMargin) : UIHelper.dip2px(mPageMargin);
        int bottomMargin = position == itemCount - 1 ? UIHelper.dip2px(mLeftPageVisibleWidth + 2 * mPageMargin) : UIHelper.dip2px(mPageMargin);
    ...
    private void onSetHoritiontalParams(ViewGroup parent, View itemView, int position, int itemCount) {
        int itemNewWidth = parent.getWidth() - UIHelper.dip2px(4 * mPageMargin + 2 * mLeftPageVisibleWidth);
        int itemNewHeight = parent.getHeight();

        mItemComusemX = itemNewWidth + UIHelper.dip2px(2 * mPageMargin);

        DebugLog.d(TAG, "onSetHoritiontalParams -->" + "parent.width=" + parent.getWidth() + ";mPageMargin=" + UIHelper.dip2px(mPageMargin) + ";mLeftVis=" + UIHelper.dip2px(mLeftPageVisibleWidth) + ";itemNewWidth=" + itemNewWidth);

        // 适配第0页和最后一页没有左页面和右页面，让他们保持左边距和右边距和其他项一样
        int leftMargin = position == 0 ? UIHelper.dip2px(mLeftPageVisibleWidth + 2 * mPageMargin) : UIHelper.dip2px(mPageMargin);
        int rightMargin = position == itemCount - 1 ? UIHelper.dip2px(mLeftPageVisibleWidth + 2 * mPageMargin) : UIHelper.dip2px(mPageMargin);

...
//    private List<BookCatalogue> directoryList = new ArrayList<>();
    ...
    public static synchronized PageFactory getInstance() {
        if (pageFactory == null) {
            pageFactory = createPageFactory(MyApplication.getInstance().mContext);

        }
        return pageFactory;
    }

    private static synchronized PageFactory createPageFactory(Context context) {
        if (pageFactory == null) {
    ...
    private PageFactory(Context context) {
        ...
        mBatterryFontSize = UIHelper.dip2px(12);
    ...
    public void onDraw(Bitmap bitmap, List<String> m_lines, Boolean updateCharter) {
        ...
        //画电池外框
        float width = UIHelper.dip2px(20) - mBorderWidth;
        float height = UIHelper.dip2px(10);
        ...
        //画电池头
        int poleHeight = UIHelper.dip2px(10) / 2;
        ...
        c.drawRect(rect2, mBatterryPaint);
        //画书名
        c.drawText(StringUtils.subString(bookName, 12), marginWidth, statusMarginBottom + mBatterryFontSize, mBatterryPaint);
        //画章
        if (getDirectoryList().size() > 0) {
            String charterName = StringUtils.subString(getDirectoryList().get(currentCharter).getBookCatalogue(), 12);
    ...
    public void setBookBg(int type) {
        ...
        switch (type) {
            case Config.BOOK_BG_DEFAULT:
                ...
                bitmap = BitmapUtil.decodeSampledBitmapFromResource(mContext.getResources(), R.drawable.paper, mWidth, mHeight);

...
public class MediaNotificationManager extends BroadcastReceiver {
    ...
    public MediaNotificationManager(MusicService service) throws RemoteException {
        ...
        // 暂定
        mPauseIntent = PendingIntent.getBroadcast(mMusicService, REQUEST_CODE, new Intent(ACTION_PAUSE).setPackage(pkg), PendingIntent.FLAG_CANCEL_CURRENT);
        // 播放
        mPlayIntent = PendingIntent.getBroadcast(mMusicService, REQUEST_CODE, new Intent(ACTION_PLAY).setPackage(pkg), PendingIntent.FLAG_CANCEL_CURRENT);
        // 上一个
        mPreviousIntent = PendingIntent.getBroadcast(mMusicService, REQUEST_CODE, new Intent(ACTION_PREV).setPackage(pkg), PendingIntent.FLAG_CANCEL_CURRENT);
        // 下一个
        mNextIntent = PendingIntent.getBroadcast(mMusicService, REQUEST_CODE, new Intent(ACTION_NEXT).setPackage(pkg), PendingIntent.FLAG_CANCEL_CURRENT);
    ...
    private PendingIntent createContentIntent(MediaDescriptionCompat description) {
        try {
            Intent intent = mMusicService.getPackageManager().getLaunchIntentForPackage(mMusicService.getPackageName()).setPackage(null).setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_RESET_TASK_IF_NEEDED);
    ...
    private Notification createNotification() {
        ...
        // 下一曲
        notificationBuilder.addAction(R.drawable.ic_skip_next_white_36dp, mMusicService.getString(R.string.label_next), mNextIntent);
        ...
        if (description.getIconUri() != null) {
            ...
            if (art == null) {
                ...
                art = BitmapFactory.decodeResource(mMusicService.getResources(), R.drawable.ic_notification);
            }
        }
    ...
    private void fetchBitmapFromURLAsync(final String bitmapUrl) {
        AlbumArtCache.getInstance().fetch(bitmapUrl, new AlbumArtCache.FetchListener() {
            ...
            public void onFetched(String artUrl, Bitmap bitmap, Bitmap icon) {
                if (mMediaMetadata != null && mMediaMetadata.getDescription().getIconUri() != null && mMediaMetadata.getDescription().getIconUri().toString().equals(artUrl)) {

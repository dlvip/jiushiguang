...
public class GlideUtils {
    ...
    public void setImageView(Context context, String url, ImageView imageView) {
        setImageView(context, url, imageView, R.drawable.piclib_shape_radius_666);
    ...
    public void setImgTransRes(Context context, String url, ImageView imageView, int W, int H) {
        setImageViewTrans(context, url, imageView, W, H, R.drawable.piclib_shape_radius_666);
    ...
    public void setImageView(Context context, String url, ImageView imageView, int width, int height) {
        setImageView(context, url, imageView, width, height, R.drawable.piclib_shape_radius_666);
    ...
    public void setImageViewWH(Context context, String url, ImageView imageView, int wh) {
        setImageView(context, url, imageView, wh, wh, R.drawable.piclib_shape_radius_666);
    ...
    public void setimageView(Context context, String url, ImageView imageView, int w, int h) {
        setImageView(context, url, imageView, w, h, R.drawable.piclib_shape_radius_666);
    ...
    public void setRoundImageView(Context context, String url, ImageView imageView) {
        setRoundImageView(context, url, imageView, R.drawable.piclib_shape_radius_666);
    ...
    public void setRadiusImageView(Context context, String url, ImageView imageView, int radius) {
        if (TextUtils.isEmpty(url)) {
            setImageView(context, imageView, R.color.pic_lib_color_666);
    ...
    public void setRoundImageView(Context context, String url, ImageView imageView, int resourceId) {
        ...
        int WH = ScreenTools.dip2px(context, 200);
        url = getPicUrl(url, WH, WH);
        if (context == null) {
    ...
    public void setVideoToImageView(Context context, String url, ImageView imageView) {
        setVideoToImageView(context, url, imageView, R.drawable.piclib_shape_radius_666);
     ...
     */
    public void downLoadBitmap(Context context, String url, int[] WH, final GlideDealUtils.ImageDownLoadCallBack downLoadCallBack) {
        RequestManager manager = getRequestManager(context);
        if (manager == null) {
        ...
        if (TextUtils.isEmpty(url)) {
            requestBuilder = manager.asBitmap().load(R.drawable.piclib_shape_radius_666);

        } else {
            requestBuilder = manager.asBitmap().load(url);

        }
        requestBuilder.apply(new RequestOptions().override(ScreenTools.dip2px(context, 50)))//
     ...
     * @param downLoadCallBack
     */
    public void downLoadBitmap(Context context, String url, int radius, final GlideDealUtils.ImageDownLoadCallBack downLoadCallBack) {
        RequestManager manager = getRequestManager(context);
        if (manager == null) {

            return;
        }
        int WH = ScreenTools.dip2px(context, 50);
        url = getPicUrl(url, WH, WH);
        RequestBuilder<Bitmap> requestBuilder;
        if (TextUtils.isEmpty(url)) {
            requestBuilder = manager.asBitmap().load(R.drawable.piclib_shape_radius_666);
        ...
        requestBuilder.apply(new RequestOptions().transform(new GlideRoundTransform(radius))//
                .override(WH)).into(new SimpleTarget<Bitmap>() {
        ...
        if (!TextUtils.isEmpty(url) && url.contains("storage")) {//手机图片
            ...
            if (!TextUtils.isEmpty(url) && "x-oss-process=image/resize".contains(url)) {//防止二次拼接后缀
                ...
                if (width == 0 || height == 0) {
                    ...
                    url = OSSURL + url + "?x-oss-process=image/resize,m_fill,w_" + width + ",h_" + height;
